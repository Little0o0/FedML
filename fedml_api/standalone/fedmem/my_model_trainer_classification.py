...
class MyModelTrainer(ModelTrainer):
        ...
        with torch.no_grad():
            for name, weight in self.model.named_parameters():
                if name not in self.penalty_index:
                    continue
                penalty += 0.1 * torch.norm(weight.flatten()[self.penalty_index[name]])
        return penalty
    ...
    def get_top_k_grad(self, k_dict):
            ...
            for batch_idx, (x, labels) in enumerate(train_data):
                ...
                if mode == 4:
                    for name, weight in self.model.named_parameters():
                        if name not in self.penalty_index:
                            continue
                        loss += 0.1 * torch.norm(weight.flatten()[self.penalty_index[name]])
                loss.backward()
                ...
                if (mode == 2 and (epoch + 1) == args.epochs
