...
class MobileBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernal_size, stride, nonLinear, SE, exp_size):
        ...
        self.conv = nn.Sequential(
            WSConv2D(in_channels, exp_size, kernel_size=1, stride=1, padding=0, bias=False),
            # nn.BatchNorm2d(exp_size),
            activation(inplace=True)
        )
        self.depth_conv = nn.Sequential(
            WSConv2D(exp_size, exp_size, kernel_size=kernal_size, stride=stride, padding=padding, groups=exp_size),
            # nn.BatchNorm2d(exp_size),
        )
        ...
        self.point_conv = nn.Sequential(
            WSConv2D(exp_size, out_channels, kernel_size=1, stride=1, padding=0),
    ...
    def __init__(self, model_mode="LARGE", num_classes=1000, multiplier=1.0, dropout_rate=0.0):
        ...
        if model_mode == "LARGE":
            ...
            self.init_conv = nn.Sequential(
                WSConv2D(in_channels=3, out_channels=init_conv_out, kernel_size=3, stride=2, padding=1),
            ...
            self.out_conv1 = nn.Sequential(
                WSConv2D(out_conv1_in, out_conv1_out, kernel_size=1, stride=1),
            ...
            self.out_conv2 = nn.Sequential(
                WSConv2D(out_conv2_in, out_conv2_out, kernel_size=1, stride=1),
                h_swish(inplace=True),
                nn.Dropout(dropout_rate),
                WSConv2D(out_conv2_out, self.num_classes, kernel_size=1, stride=1),
            )
